...
#define GE_LOG_TRACE(msg) GE::Utilities::Logger::Trace(msg, __FILE__, __LINE__)
#define GE_LOG_DEBUG(msg) GE::Utilities::Logger::Debug(msg, __FILE__, __LINE__)
#define GE_LOG_INFO(msg) GE::Utilities::Logger::Info(msg, __FILE__, __LINE__)
#define GE_LOG_WARN(msg) GE::Utilities::Logger::Warn(msg, __FILE__, __LINE__)
#define GE_LOG_ERROR(msg) GE::Utilities::Logger::Error(msg, __FILE__, __LINE__)
#ifndef NDEBUG
#define GE_LOG_FATAL(msg)														\
    do																			\
    {																			\
        GE::Utilities::Logger::Fatal(msg, __FILE__, __LINE__);					\
        assert(!"FATAL ERROR! Look call stack and engine.log!");				\
    }                                                                           \
    while (0)
#else
#define GE_LOG_FATAL(msg) GE::Utilities::Logger::Fatal(msg, __FILE__, __LINE__)
#endif

#define GE_LOG(error_code, msg)								\
	do {													\
		const auto ec_ = (error_code);						\
		if (ec_ < GE::ERROR_CODE::ERROR_START) {			\
			GE_LOG_FATAL(msg);								\
		} else if (ec_ < GE::ERROR_CODE::WARN_START) {		\
			GE_LOG_ERROR(msg);								\
		} else if (ec_ < GE::ERROR_CODE::DEBUG_START) {		\
			GE_LOG_WARN(msg);								\
		} else if (ec_ < GE::ERROR_CODE::TRACE_START) {		\
			GE_LOG_DEBUG(msg);								\
		} else if (ec_ < GE::ERROR_CODE::OK_START) {		\
			GE_LOG_TRACE(msg);								\
		} else {											\
			GE_LOG_INFO(msg);								\
		}													\
...
#define GE_CHECK(var, expr)									\
	do {									                \
		(var) = (expr);										\
		if ((var) < GE::ERROR_CODE::WARN_START) {			\
			return (var);									\
		}													\
	} while (0)

#define GE_CHECK_ALREADY_INIT(flag, msg)					\
	do {													\
		if (flag) {											\
			GE_LOG_FATAL(msg);								\
			return GE::ERROR_CODE::ALREADY_INITIALIZED;		\
		}													\
	} while (0)

#define GE_CHECK_STATE_INIT(state, msg)						\
	do {													\
		if ((state) != GE::SystemState::Uninitialized) {	\
			GE_LOG_FATAL(msg);								\
			return GE::ERROR_CODE::ALREADY_INITIALIZED;		\
		}													\
	} while (0)

#define GE_ENSURE_INIT(var, expr, msg)						\
	do {													\
		(var) = (expr);										\
		if ((var) < GE::ERROR_CODE::WARN_START) {			\
			GE_LOG((var), msg);								\
...
#define GE_ENSURE_INIT_SILENT(var, expr)					\
	do {													\
		(var) = (expr);										\
		if ((var) < GE::ERROR_CODE::WARN_START) {			\
			Shutdown();										\
...
#define GE_NULLIFY(var_1, var_2, expr)						\
do {														\
	(var_1) = (expr);										\
	if ((var_1) < GE::ERROR_CODE::WARN_START) {				\
		delete (var_2);										\
		(var_2) = nullptr;									\
		return (var_1);										\
	}														\
...
private:
	...
	static std::string FormatMessage(LogLevel level, const std::wstring &msg, const char *file, const int line);
